...
class FriendRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return FriendRequset.objects.filter(Q(sender=user) | Q(receiver=user))
    ...
    def incoming_request(self, request):
        qs = FriendRequset.objects.filter(receiver=request.user, status=FriendRequset.STATUS_PENDING)
        serializer =self.get_serializer(qs, many=True)
        return Response(serializer.data)
    @action(detail=False, methods=['get'], url_path='outgoing')
    def outgoing_requests(self, request):
        qs = FriendRequset.objects.filter(sender=request.user, status=FriendRequset.STATUS_PENDING)
        serializer = self.get_serializer(qs, many=True)
        return Response(serializer.data)
    @action(detail=True, methods=['post'], url_path='accept')
    def accept(self, request, pk=None):
        friend_request = self.get_object()
        if friend_request.recevier != request.user:
...
class FriendshipViewSet(viewsets.ViewSet):
    ...
    @action(detail=True, methods=['detail'], url_path='remove')
    def remove_friend(self, requst, pk=None):
        friend = get_object_or_404(settings.AUTH_USER_MODEL, pk=pk)
        Friendship.objects.filter(
            Q(user1=requst.user, user2=friend) | Q(user1=friend, user2=requst.user)
        ).delete()
...
class FollowViewSet(viewsets.ViewSet):
    ...
    def following(self, request):
        ...
        serializer = FolloweSerializer(qs, manny=True)
        return Response(serializer.data)
